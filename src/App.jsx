...
function formatTime(ms) {
  ...
  const min=Math.floor(ms/60000), sec=Math.floor((ms%60000)/1000), mil=ms%1000;
...
function Td({children, right, center, mono, bold, dim, gold}) {
  return <td style={{
    ...
    fontWeight:bold?600:400, fontSize:"0.875rem",
...
function CumulTable({ranking, data}) {
  return (
    <div style={{overflowX:"auto"}}>
      <table style={{width:"100%", borderCollapse:"collapse"}}>
        <thead><tr>
          <Th center>Pos</Th><Th center>#</Th><Th>Pilote</Th><Th>Écurie</Th>
...
function PtsTable({ranking, courses, data}) {
  return (
    <div style={{overflowX:"auto"}}>
      <table style={{width:"100%", borderCollapse:"collapse"}}>
        <thead><tr>
          <Th center>Pos</Th><Th center>#</Th><Th>Pilote</Th><Th>Écurie</Th><Th right>Points</Th>
...
function SheetsLoader({onLoad}) {
  const [url,setBusy_url] = useState("");
  ...
  return (
    <div style={{marginTop:12}}>
      <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
        <input value={url} onChange={e=>setBusy_url(e.target.value)}
...
=======
              <div style={{color:"#FFFFFF", fontSize:"0.65rem", fontWeight:700, letterSpacing:"0.18em", marginBottom:5}}>CLASSEMENT OFFICIEL</div>
...
>>>>>>> e9f29b2 (sessions essais/qualif/course + colonne position)
              </h1>
            </div>
            {champion && (
      ...
      <div style={{maxWidth:960, margin:"0 auto", padding:"24px 16px"}}>
        <SessionView key={activeSession} sessionData={sessionData} isRace={activeSession==="course"} />
      </div>

      {/* FOOTER */}
      <div style={{textAlign:"center", padding:"14px", color:C.soft, fontSize:"0.7rem", borderTop:`1px solid ${C.border}`}}>
        {isMock?"Mode démonstration":`${data.length} entrées · ${[...new Set(data.map(d=>d.pilote))].length} pilotes`}
      </div>
    </div>
  );
}
